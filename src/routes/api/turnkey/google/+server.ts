...
export async function POST({ request }) {
	try {
		const { id_token: idToken, pubkey } = await request.json();
		...
		const suborgIds = subOrgsIds?.organizationIds;
		if (!suborgIds?.length) {
			console.error('suborg failure', suborgIds);
			return json({ error: 'suborg failure' }, { status: 400 });
		}
		...
		if (!loginResp?.session) {
			console.error('oauthLogin failed', loginResp);
			return json({ error: 'oauthLogin failed' }, { status: 400 });
		}
